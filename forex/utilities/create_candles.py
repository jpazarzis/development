...
'''
Creates hourly candlesticks based on tick data
'''
...
def create_candles(instrument):
    ...
    open_time = None
    output_file.write('timestamp,count,open,high,close,low\n')
    ...
    for i, line in enumerate(input_file):
        ...
        if timestamp > end_time:
            if len(ticks) > 0:
                output_file.write('{0},{1},{2},{3},{4},{5}\n'.format(open_time,len(ticks), ticks[0], max(ticks),ticks[-1], min(ticks)))
            open_time = datetime.datetime(timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute, 0)
            end_time = datetime.datetime(timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute, 59)
            ticks = []            
        ticks.append(bid)
...
def create_hourly_candles(instrument):
    print 'creating candles for {0} '.format(instrument)
    input_filename = '{0}/{1}.csv'.format(history_ticks_dir,instrument)
    output_filename = '{0}/{1}_hourly_candles.csv'.format(history_candles_dir,instrument)
    input_file = open(input_filename)
    output_file = open(output_filename, 'w')
    open_time = None
    output_file.write('timestamp,count,open,high,close,low\n')
    ...
    for i, line in enumerate(input_file):
        ...
        if open_time is None:
            open_time = datetime.datetime(timestamp.year, timestamp.month, timestamp.day, timestamp.hour, timestamp.minute, 0)
            end_time = datetime.datetime(timestamp.year, timestamp.month, timestamp.day, timestamp.hour, 59, 59)
            ticks = []            
