...
class SellBasedInDelta: public Model
{
    ...
    public:
        ...
        {
            std::string strg;
            strg += sformat("id:", "%20s"); strg += sformat((int)_id,"%20d"); strg += "\n";
            strg += sformat("minute to trade:", "%20s"); strg += sformat((int)_minute_to_buy,"%20d"); strg += "\n";
            strg += sformat("expriration minutes:", "%20s"); strg += sformat((double)_expriration_minutes, "%20.5f"); strg += "\n";
            strg += sformat("delta:", "%20s"); strg += sformat((double)_triggering_delta, "%20.5f"); strg += "\n";
            strg += sformat("stop loss:", "%20s"); strg += sformat((double)_stop_loss, "%20.5f"); strg += "\n";
            strg += sformat("profit take:", "%20s"); strg += sformat((double)_take_profit, "%20.5f"); strg += "\n";
            strg += sformat("number of orders:", "%20s"); strg += sformat(orders_count(),"%20d"); strg += "\n";
            strg += sformat("final balance:", "%20s"); strg += sformat(get_account_balance(), "%20.2f"); strg += "\n";
            strg += sformat("final pnl:", "%20s"); strg += sformat(get_pnl(), "%20.2f"); strg += "\n";
            strg += sformat("low:", "%20s"); strg += sformat(get_absolute_low(), "%20.2f"); strg += "\n";
            strg += sformat("winning trades:", "%20s"); strg += sformat(get_winning_trades_count(), "%20d"); strg += "\n";
            strg += sformat("losing trades:", "%20s"); strg += sformat(get_loosing_trades_count(), "%20d"); strg += "\n";
            strg += sformat("expired trades total:", "%20s"); strg += sformat(get_expired_trades_count(), "%20d"); strg += "\n";
            strg += sformat("winning expired count:", "%20s"); strg += sformat(get_winning_expired_trades_count(), "%20d"); strg += "\n";
            strg += sformat("losing expired count:", "%20s"); strg += sformat(get_losing_expired_trades_count(), "%20d"); strg += "\n";
            strg += sformat("max drawdown:", "%20s"); strg += sformat(get_max_drawdown(), "%20.5f"); strg += "\n";
            return strg;
        }
